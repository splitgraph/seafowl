...
fn build_region<'a, I>(region_columns: I) -> SeafowlRegion
where
...
fn build_table<'a, I>(table_name: &str, table_columns: I) -> (Arc<str>, Arc<SeafowlTable>)
where
...
impl Catalog for PostgresCatalog {
    async fn load_database(mut self, database_id: DatabaseId) -> SeafowlDatabase {
        ...
        let collections: HashMap<Arc<str>, Arc<SeafowlCollection>> = all_columns
            ...
            .into_iter()
            .map(|(cn, cc)| build_collection(cn, cc))
            .collect();
