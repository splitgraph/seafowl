...
pub async fn record_batches_to_iceberg(
    ...
    let (previous_metadata, previous_metadata_location) = match old_version_hint {
        ...
        None => {
            let empty_metadata =
                create_empty_metadata(&iceberg_schema, target_url.to_string())?;
            (empty_metadata, None)
        }
    };
    ...
    let manifest_file_path =
        format!("{}/metadata/manifest-{}.avro", target_url, Uuid::new_v4());
    ...
    pub async fn plan_to_iceberg_table(
        ...
        for i in 0..plan.output_partitioning().partition_count() {
            ...
            record_batches_to_iceberg(
                stream.map_err(|_| {
                    DataLoadingError::BadInputError("Datafusion error".to_string())
                }), // TODO: improve
                schema,
                Url::parse(metadata_location.unwrap()).unwrap(), // Wrong? metadata_location vs table base URL
                false,
                false,
            )
            .await
